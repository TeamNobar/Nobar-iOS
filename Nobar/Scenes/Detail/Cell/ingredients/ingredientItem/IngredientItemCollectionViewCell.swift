...
class IngredientItemCollectionViewCell: UICollectionViewCell {
  ...
  func setData(cocktailData: Ingredient) {
    ingredientsTitle.text = cocktailData.name
    categoryLabel.text = cocktailData.category
    proofLabel.text = cocktailData.quantity
  }
}
